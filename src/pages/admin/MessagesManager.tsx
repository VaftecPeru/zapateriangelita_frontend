...
const MessagesManager = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {articleOrder && <div className="fixed inset-0 z-[3100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/60" onClick={() => setArticleOrder(null)} /><div className="relative bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden"><div className="bg-store-red/5 px-8 py-6 flex justify-between items-center"><div><h3 className="text-xl font-black text-black">Artículos pedidos</h3><p className="text-[10px] text-store-red font-bold uppercase tracking-widest mt-1">Pedido {articleOrder.code}</p></div><button onClick={() => setArticleOrder(null)} className="p-2 bg-white rounded-full"><X size={18} /></button></div><div className="p-8 space-y-3">{articleOrder.items?.length ? articleOrder.items.map((item, index) => <div key={`${articleOrder.id}-${index}`} className="border-b border-gray-100 pb-4 last:border-0"><div className="flex items-start justify-between gap-4"><span className="text-sm font-bold text-black">{item.product_name || 'Producto'}</span><span className="shrink-0 rounded-lg bg-gray-100 px-3 py-1 text-xs font-black text-gray-800">x{item.quantity}</span></div><div className="mt-2 flex flex-wrap gap-2 text-[11px] text-gray-800"><span className="rounded-lg bg-gray-100 px-2 py-1">Talla: {item.size || 'No especificada'}</span><span className="rounded-lg bg-gray-100 px-2 py-1">Color: {item.color || 'No especificado'}</span></div>{item.unit_price !== undefined && <p className="mt-2 text-[11px] text-gray-700">Precio unitario: {money.format(Number(item.unit_price))}</p>}</div>) : <p className="text-sm text-gray-700">No hay artículos registrados en este pedido.</p>}</div></div></div>}
        </div>
    );
};
