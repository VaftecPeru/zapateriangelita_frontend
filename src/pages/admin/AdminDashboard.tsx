...
import PropertyManager from './ProductManager';
            ...
            <div className="flex-1 flex items-center justify-center relative w-full h-[220px]">
                <div className="relative flex items-center justify-center w-56 h-56 sm:w-64 sm:h-64 -ml-16 sm:-ml-24">
                    ...
                    {layers.slice(0, 3).map((layer, i) => (
                        ...
                        >
                            <span className={`text-[10px] sm:text-[11px] font-black bg-white/60 px-2 py-1 rounded backdrop-blur-sm ${i === 0 ? 'text-store-red/60' : i === 1 ? 'text-store-red/80' : 'text-store-red'}`}>
                                {layer.label}
                            </span>
                        </div>
                    ))}
            ...
            <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 opacity-80">
                        <div className="w-2 h-2 rounded-full bg-store-red"></div>
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    <div className="flex items-end justify-between h-24 gap-1.5 px-1 border-b border-gray-100 pb-1">
                        {bars.map((val: number, i: number) => (
                            <div key={i} className="w-full flex justify-center h-full items-end group">
                                <div
                                    className={`w-full max-w-[20px] rounded-t-lg transition-all duration-1000 ${i === bars.length - 1 ? 'bg-store-red' : 'bg-store-red/20 group-hover:bg-store-red/40'}`}
                    ...
                    <div className="flex items-center justify-between px-1 mt-2 text-[9px] font-black text-black/60 tracking-wider">
                        {labels.map((lbl: string, i: number) => (
                            <span key={i} className={`flex-1 text-center ${i === labels.length - 1 ? 'text-store-red border-b-2 border-store-red' : ''}`}>
                                {lbl}
                            </span>
                        ))}
                    </div>
...
const AdminDashboard = () => {
    ...
    const statCards = [
        ...
        },
        {
            ...
            icon: <ArrowDownRight className="text-store-red" />
        },
    ];

    return (
        <div className="min-h-screen bg-store-surface pt-6 md:pt-12 pb-12 px-4 md:px-8 lg:px-12">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`px-6 py-3 text-sm font-black uppercase tracking-widest transition-all border-b-2 whitespace-nowrap ${activeTab === tab.id
                                ? 'border-store-red text-store-red font-bold'
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
                        ...
                        <div className="lg:col-span-8 space-y-6 md:space-y-8">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div className="bg-store-ink rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 text-white relative overflow-hidden group shadow-lg">
                                    <div className="relative z-10 space-y-6">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div className="flex -space-x-2">
                                                <div className="w-8 h-8 rounded-full border-2 border-black bg-store-red" />
                                        ...
                                        <div className="flex justify-between items-end pt-4 border-t border-white/10">
                                            <div className="text-[10px] font-black tracking-widest uppercase text-white/40">ZAPATERÍA ANGELITA</div>
                                            <button
                                    ...
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-store-red/20 rounded-full -mr-16 -mt-16 blur-3xl group-hover:bg-store-red/40 transition-colors" />
                                </div>
                                    ...
                                    <div key={i} className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 group">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="flex items-center gap-1 text-xs font-bold">
                                                {stat.icon}
                                                <span className={stat.change.startsWith('+') ? 'text-green-600' : 'text-store-red'}>{stat.change}</span>
                                            </div>
                                ...
                                <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border border-gray-100 shadow-sm">
                                    ...
                                    <div className="flex items-center justify-center py-10">
                                        <div className="relative w-48 h-48">
                                            ...
                                            <div className="absolute inset-0 rounded-full border-[16px] border-store-red border-t-transparent border-l-transparent rotate-45" />
                                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                <span className="text-3xl font-black text-black">
                                                    <Counter value={stats?.charts?.benefitsDistribution?.total || 0} prefix="" suffix="" decimals={0} />
                                                </span>
                                                <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Productos</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-6 flex-1">
                                        {(stats?.recentActivity || []).map((item: any, i: number) => (
                                            <div key={i} className="flex gap-4 group cursor-pointer">
                                                ...
                                                <div className="space-y-1">
                                                    <p className="text-sm font-bold text-black leading-tight group-hover:text-store-red transition-colors">{item.text}</p>
